...
**New short IDs default to generator-picked random values.** Draw an ID with `scripts/new-short-url.py --generate-only`, which returns a collision-free 2-char base62 value without touching the registry; the draw skips YAML 1.1 boolean words and ISO 639-1 language codes in any casing, so an ID never reads as a language (`/s/en` and `/s/EN` both look like English) and never reloads as a boolean key. Reserve hand-chosen memorable IDs for the case where a person will type the URL from memory (a mnemonic landing alias like `pr`), not for URLs the app or a button builds mechanically.

**A short URL whose `redirect_to` targets a project page that carries its own custom social banner reuses that project's branding.** `generate_share.py` detects this from `redirect_to` (a `/projects/<slug>.html` path whose front matter declares banner fields and an icon) and composes the project's icon and banner title and subtitle with the entry's `bannertagline` as the drawn tagline, instead of the default personal face card. Keep the entry's `title` equal to the project's banner title so the card title and the banner agree, and let `description` and `bannertagline` carry the per-target differentiation.

**A project-branded share can localize its subtitle, not the brand line.** An entry whose `redirect_to` lands on a project page may set an optional `bannersubtitle`, drawn under the banner title in place of the project's own subtitle. There is no `bannertitle` override: brand names are not translated. Aliases may not set `bannersubtitle` or `bannertagline` (schema-enforced) and inherit the target's banner. The Network Nudge template pins exercise this: each template's `pinnedShareIds: { en, fr }` in `templates.js` maps the compose share sheet's locale-locked flavor (`#<locale>--<templateId>`) to a per-locale registry ID, with the `en` pins as `same_as` aliases over the bare English entries and the `fr` pins as full entries owning baked French banners. `scripts/validate-short-urls.py` cross-checks the pinned IDs against the registry.
