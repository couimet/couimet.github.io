...
# Requires: Docker (required), Node (required, reads the pinned tool versions
# from package.json), npm (optional, for version-check; skipped gracefully if absent)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
# Pinned tool versions live in package.json (repo root) as exact-pinned
# devDependencies so Dependabot can bump them; the Makefile targets print them
# so every consumer reads the same source.
